...
SEND_MESSAGE_SCHEMA = {
    ...
    "description": (
        ...
        "Do not use this tool to reply or attach a file in the current conversation; "
        "put the text and any MEDIA:/absolute/path lines in your final response so "
        "the gateway delivers them back to the originating chat/thread.\n"
    ...
    try:
        ...
        for media_path, is_voice in media_files:
            if not os.path.exists(media_path):
                warning = _missing_media_warning(media_path)
...
def _missing_media_warning(media_path: str) -> str:
    """Explain the common host/Docker path mismatch for missing media files."""
    warning = f"Media file not found, skipping: {media_path}"
    normalized = os.path.abspath(os.path.expanduser(str(media_path)))
    if normalized.startswith("/tmp/") or normalized.startswith("/root/.hermes/"):
        basename = os.path.basename(normalized) or "<filename>"
        warning += (
            ". If this file was generated by the terminal tool in Docker, "
            "copy it to /home/rj/.hermes/out/{name} inside the terminal "
            "container and resend MEDIA:/home/rj/.hermes/out/{name}; "
            "Telegram delivery runs on the host and cannot read Docker-only "
            "/tmp or /root/.hermes paths."
